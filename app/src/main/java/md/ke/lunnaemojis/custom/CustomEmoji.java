...
public class CustomEmoji extends Emoji {
  ...
  public CustomEmoji(@NonNull final int[] codePoints, @NonNull final String[] shortcodes, final int x, final int y,
  ...
  public CustomEmoji(final int codePoint, @NonNull final String[] shortcodes, final int x, final int y,
                     final boolean isDuplicate) {
  ...
  private Bitmap loadStrip(final Context context) {
    ...
    if (strip == null) {
      synchronized (LOCK) {
        ...
        if (strip == null) {
          String name = "/emoji_"+ App.getCustomEmojiName()+"_" + x + ".png";
          strip = BitmapFactory.decodeFile(Files.getCustomEmojiDirectory()+name);
